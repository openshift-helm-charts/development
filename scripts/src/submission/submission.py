    ...
    * by providing all class attributes. This is typically done by loading a JSON representation of
    ...
    def set_tarball(
        ...
        if file_extension == ".tgz":
            ...
            expected_directory = f"{chart_name}-{chart_version}"
            check_tarball_content(
                os.path.join(repo_path, file_path), expected_directory
            )
...
def check_tarball_content(tarball_path: str, expected_directory: str):
    """Check the tarball content for errors.

    Checks that the tarball contains a unique directory named after the chart and version. This directory must contain
    a Chart.yaml file. The directory may contain additional files or folders. No other files or folder should be placed
    at the root of the archive.

    Args:
        tarball_path (str): Location of the tarball to check on the local filesystem.
        expected_directory (str): Name of the directory expected at the root of the tarball.
    ...
    expected_chart_file_path = os.path.join(expected_directory, "Chart.yaml")
    with tarfile.open(tarball_path, "r:gz") as tar:
        tarinfo = tar.next()
        while tarinfo:
            if tarinfo.isdir() and tarinfo.name == expected_directory:
                found_chart_directory = True
            elif not tarinfo.name.startswith(expected_directory + "/"):
    ...
    if not found_chart_directory:
        msg = f"[ERROR] Incorrect tarball content: expected a {expected_directory} directory"
        raise TarballContentError(msg)

    if found_file_out_of_dir:
        msg = f"[ERROR] Incorrect tarball content: found a file outside the {expected_directory} directory"
        raise TarballContentError(msg)
