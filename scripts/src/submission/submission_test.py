...
scenarios_submission_init = [
    ...
    SubmissionInitScenario(
        ...
        tarball_content=[
            os.path.join(f"{expected_name}-{expected_version}", "Chart.yaml")
        ],
    ...
    SubmissionInitScenario(
        ...
        tarball_content=[
            os.path.join(f"{expected_name}-{expected_version}", "Chart.yaml"),
            os.path.join(f"{expected_name}-{expected_version}", "bar"),
            os.path.join(f"{expected_name}-{expected_version}", "foo", "baz"),
        ],
        expected_submission=make_new_tarball_only_submission(),
    ),
    # PR contains an unsigned tarball
    ...
    SubmissionInitScenario(
        ...
        excepted_exception=pytest.raises(
            ...
            match=re.escape(
                f"[ERROR] Incorrect tarball content: expected a {expected_name}-{expected_version} directory"
            ),
        ),
    ),
    # PR contains an unsigned tarball
    ...
    SubmissionInitScenario(
        ...
        excepted_exception=pytest.raises(
            submission.SubmissionError,
            match=re.escape(
                f"[ERROR] Incorrect tarball content: found a file outside the {expected_name}-{expected_version} directory"
            ),
        ),
    ),
    # PR contains an unsigned tarball
    ...
    SubmissionInitScenario(
        ...
        excepted_exception=pytest.raises(
            submission.SubmissionError,
            match=re.escape(
                f"[ERROR] Incorrect tarball content: expected a {expected_name}-{expected_version}/Chart.yaml file"
            ),
        ),
    ),
    # PR contains an unsigned tarball
